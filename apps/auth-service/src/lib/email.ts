...
export async function sendVerificationEmail(
  ...
  const mailOptions = {
    from: `"FlexWork" <${fromAddress}>`,
    to,
    subject: 'Verify your email – FlexWork',
    html: `
      <!DOCTYPE html>
      <html>
      <head>
          ...
          <tr>
            <td align="center">
              <table width="100%" cellpadding="0" cellspacing="0" style="max-width:480px;background-color:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 4px 6px rgba(0,0,0,0.07);">
                ...
                <tr>
                  <td style="padding:32px;">
                    <h2 style="margin:0 0 8px;font-size:20px;color:#18181b;">Verify your email address</h2>
                    <p style="margin:0 0 24px;color:#71717a;font-size:14px;line-height:1.6;">
                      Hi <strong style="color:#18181b;">${firstName}</strong>, thanks for signing up! Please verify your email address to activate your account.
                    </p>
                    <table cellpadding="0" cellspacing="0" border="0" style="margin:0 auto;">
                      <tr>
                        <td align="center" bgcolor="#6366f1" style="border-radius:8px;">
                          <a href="${verifyLink}" target="_blank" style="display:inline-block;background:linear-gradient(135deg,#6366f1,#8b5cf6);color:#ffffff;text-decoration:none;padding:14px 36px;border-radius:8px;font-weight:600;font-size:15px;letter-spacing:0.3px;">
                            Verify Email
                          </a>
                        </td>
                      </tr>
  ...
  try {
    const transport = getTransporter();
    const info = await transport.sendMail(mailOptions);
    logger.info(`Verification email sent to ${to.replace(/(.{2}).*(@.*)/, '$1***$2')}, messageId: ${info.messageId}`);
  } catch (error) {
